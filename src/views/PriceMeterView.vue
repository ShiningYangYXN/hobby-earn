...
const selectedMemberId = computed<string | null>({
  ...
  set: (id) => {
    selectedMember.value = id ? (memberStore.members.find((m) => m.id === id) ?? null) : null
  },
})
...
const eligibleNonCoupon = computed<Discount[]>(() => {
  ...
  return discountStore.discounts.filter(
    (d) =>
      ...
      !!discountStore.calcDiscount(d, subtotal.value, memberId, completed, curMemberTypeId.value),
  )
})
const appliedNonCoupon = computed<Discount[]>(() =>
  eligibleNonCoupon.value.filter((d) => !excludedNonCoupon.value.includes(d.id)),
)
const discIds = computed<string[]>(() => [
  ...appliedNonCoupon.value.map((d) => d.id),
  ...couponIds.value,
])
...
function discountAmountOf(d: Discount): number {
  const memberId = curMemberId.value
  return (
    discountStore.calcDiscount(
      d,
      subtotal.value,
      memberId,
      completedCountOf(memberId),
      curMemberTypeId.value,
    )?.amount ?? 0
  )
}
...
function applyCode(raw?: string) {
  ...
  if (code.length < 6) {
    if (!raw) msg.warning('请输入 6 位券码')
    return
  }
  const res = discountStore.checkCode(code, curMemberId.value)
  if (!res.ok) {
    codeValue.value = []
    msg.error(res.reason ?? '券码不可用')
    return
  }
  const d = discountStore.findByCode(code)!
  if (couponIds.value.includes(d.id)) {
    codeValue.value = []
    return
  }
  const g = d.exclusiveGroup?.trim()
  if (g) {
    const conflict = discountStore.discounts.find(
      (x) => x.id !== d.id && x.exclusiveGroup?.trim() === g && discIds.value.includes(x.id),
    )
    if (conflict) {
      codeValue.value = []
      msg.error('该券与已用优惠互斥，无法叠加')
      return
    }
  }
...
const discRecords = computed(() => {
  ...
  for (const id of resolvedDiscIds.value) {
    const d = discountStore.discounts.find((x) => x.id === id)
    if (!d) continue
    const r = discountStore.calcDiscount(
      d,
      subtotal.value,
      memberId,
      completed,
...
async function bookOrder() {
  ...
  try {
    const items = cart.value.map((i) =>
      i.pricingMode === 'hourly' ? { ...i, elapsed: timerElapsed.value[i.priceEntryId] ?? 0 } : i,
    )
...
async function openOrderInner(o: Order) {
  ...
  couponIds.value = fresh.discountRecords
    .filter(
      (r) => discountStore.discounts.find((d) => d.id === r.discountId)?.discountType === 'coupon',
    )
    .map((r) => r.discountId)
  excludedNonCoupon.value = []
...
async function pauseExec() {
  ...
  const items = execItems.value.map((i) =>
    i.pricingMode === 'hourly' ? { ...i, elapsed: timerElapsed.value[i.priceEntryId] ?? 0 } : i,
  )
  stopAllTimers()
  try {
...
async function finalizeOrder() {
  if (!activeOrder.value) return
  const items = execItems.value.map((m) =>
    m.pricingMode === 'hourly' ? { ...m, elapsed: timerElapsed.value[m.priceEntryId] ?? 0 } : m,
  )
  try {
    ...
    <template v-if="isExecuting && activeOrder">
      ...
      <NCard size="small" title="服务项目">
        <NSpace vertical :size="2">
          <div
            v-for="(item, i) in execItems"
            :key="item.priceEntryId + '-' + i"
            style="
              display: flex;
              align-items: center;
              gap: 10px;
          ...
          >
            ...
            <div
              v-if="item.pricingMode === 'perPiece'"
              style="
                display: flex;
                align-items: center;
                gap: 6px;
            ...
            >
              ...
              <span style="font-size: 13px; min-width: 20px; text-align: center">{{
                item.quantity
              }}</span>
            ...
            >
              <span
                ...
                "
              >
                预订 {{ formatTime(item.elapsed ?? 0) }} · 实际
                {{ formatTime(timerElapsed[item.priceEntryId] ?? 0) }}
              </span>
              <NButton
                size="tiny"
                circle
                :type="timerRunning[item.priceEntryId] ? 'warning' : 'success'"
                @click="
                  timerRunning[item.priceEntryId]
                    ? pauseTimer(item.priceEntryId)
                    : startTimer(item.priceEntryId)
                "
              >
                <NIcon>
            ...
            <span
              style="font-weight: 600; min-width: 64px; text-align: right"
        ...
        <div v-if="resolvedDiscIds.length" style="margin-top: 8px">
          <NTag
            v-for="r in discRecords"
            :key="r.id"
            type="warning"
            size="tiny"
            closable
            style="margin-right: 4px"
            @close="removeDisc(r.id, r.type)"
            >{{ r.desc }}</NTag
          >
        </div>
        ...
        <NSpace vertical :size="4">
          <NSpace justify="space-between"
            ><span>小计</span><span>¥{{ fmt(subtotal) }}</span></NSpace
          >
          <NSpace
            v-if="discRecords.length"
      ...
      <NCard size="small">
        <NSpace vertical :size="10">
          <span style="color: var(--n-text-color-3)"
            >计价器用于实际执行已确认的订单（真实计时）。点击下方订单开始，或新建一笔订单。</span
          >
          <div
            v-if="orderStore.executableOrders.length === 0"
            style="color: var(--n-text-color-3); font-size: 13px"
          >
            暂无可执行订单（请先在订单页确认订单）。
          </div>
          <NCard
            v-for="o in orderStore.executableOrders"
            :key="o.id"
            size="small"
      ...
      <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 16px">
        <NCard size="small">
          ...
          <NSpace vertical :size="6" style="max-height: 360px; overflow-y: auto">
            <NCard
              v-for="p in priceStore.activePrices"
              :key="p.id"
              size="small"
              style="display: flex; align-items: center; justify-content: space-between"
            >
        ...
        <NCard size="small" title="购物车（预计）">
          ...
          <template v-else #default>
            <NSpace vertical :size="2">
              ...
              >
                ...
                >
                  ...
                  <NInputNumber
                    :value="Math.floor((timerElapsed[item.priceEntryId] ?? 0) / 60)"
                    @update:value="
                      (v) =>
                        setDur(
                          item.priceEntryId,
                          (v ?? 0) * 60 + ((timerElapsed[item.priceEntryId] ?? 0) % 60),
                        )
                    "
                    :min="0"
                    :step="5"
                  ...
                  <NInputNumber
                    :value="(timerElapsed[item.priceEntryId] ?? 0) % 60"
                    @update:value="
                      (v) =>
                        setDur(
                          item.priceEntryId,
                          Math.floor((timerElapsed[item.priceEntryId] ?? 0) / 60) * 60 + (v ?? 0),
                        )
                    "
                    :min="0"
                    :max="59"
            ...
            <NSpace vertical :size="4">
              ...
              >
                <span>优惠</span
                ><span>-¥{{ fmt(discRecords.reduce((s, r) => s + r.amount, 0)) }}</span>
              </NSpace>
              <NSpace justify="space-between" style="font-weight: 700; font-size: 16px">
                <span>预计实收</span>
                <span style="color: var(--n-success-color)">¥{{ fmt(finalAmount) }}</span>
    ...
    <NModal v-model:show="showDisc" title="优惠" preset="card" style="width: 380px">
      <NSpace vertical :size="12">
        <div v-if="eligibleNonCoupon.length">
          <div style="font-size: 12px; color: var(--n-text-color-3); margin-bottom: 6px">
            自动适用优惠（可手动关闭）
          </div>
          <NSpace vertical :size="6">
            <div
              v-for="d in eligibleNonCoupon"
              :key="d.id"
              style="display: flex; align-items: center; gap: 8px"
            >
              <NCheckbox
                :checked="!excludedNonCoupon.includes(d.id)"
        ...
        <div>
          ...
          <div style="font-size: 12px; color: var(--n-text-color-3); margin-top: 6px">
            输满 6 位自动兑换，兑换后自动关闭
          </div>
        </div>
      </NSpace>
