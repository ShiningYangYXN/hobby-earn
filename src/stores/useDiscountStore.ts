...
export const useDiscountStore = defineStore('discount', () => {
  ...
  async function create(d: Omit<Discount, 'id' | 'usedCount' | 'memberUsedCount'>): Promise<Discount> {
    const item: Discount = {
      ...
      code: d.discountType === 'coupon' ? (d.code && d.code.trim() ? d.code.toUpperCase() : genCode()) : undefined,
    }
  ...
  function checkCode(code: string, memberId?: string): { ok: boolean; reason?: string } {
    ...
    if (d.usageLimit !== null && d.usedCount >= d.usageLimit) return { ok: false, reason: '券码已达使用上限' }
    if (memberId && d.memberLimit !== null && (d.memberUsedCount[memberId] ?? 0) >= d.memberLimit) return { ok: false, reason: '您已使用过该券' }
    return { ok: true }
  }
  ...
  return { discounts, activeDiscounts, load, create, update, recordUsage, rollbackUsage, remove, findByCode, isValidCode, checkCode, calcDiscount }
})
