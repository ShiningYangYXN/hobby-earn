...
订单项落库时固化计费方式（`pricingMode`）与单价（工时＝时薪，按件＝件单价）。服务在「服务管理」里改动计价方式后按下述规则生效：**未完成订单（待处理 / 执行中）与后续订单使用新的计价方式**，计价器、订单详情与订单列表会同步对齐并标注「计费方式已更新」，直到按新方式保存 / 完成计价；**已完成 / 已关闭订单属于历史账目，永久沿用执行时的计费方式**，不受后续服务配置改动影响。方式切换的换算规则见 `types.ts` 的 `alignItemToService`：改为按工时须重新计时（件数无法换算成工时），改为按件则按已计时长折算件数（向上取整，至少 1）。
